...
const FoodDetail = ({ onClose }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://127.0.0.1:8000/food/${food_slug}/`)
  ...
  const fetchReviews = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/food/${food_slug}/reviews/`)
  ...
  const updateDisplayedReviews = (allReviews, page, tab, sort) => {
    ...
    if (tab !== "all") {
      ...
      filteredReviews = allReviews.filter((review) => review.rating === ratingFilter)
    }
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      if (isLoggedIn) {
        ...
        // Anonymous review
        await axios.post(`http://127.0.0.1:8000/food/${food_slug}/reviews/anonymous/`, {
          rating: userRating,
          comment: comment,
      ...
      <div className="food-detail-page">
        <div className="food-image-container">
          <div className="food-image-section">
            <img src={food.food_img_url || images.defaultFoodImage} alt={food.food_name} />
            {food.food_type?.food_type && <span className="food-badge">{food.food_type.food_type}</span>}
          </div>
        </div>
        <div className="food-details">
          ...
          <div className="food-rating">
            <div className="rating-summary">
              ...
              <div className="stars">
                {[...Array(5)].map((_, i) => (
                  <span key={i} className={i < Math.floor(calculateAverageRating()) ? "star filled" : "star"}>
                    ★
                  </span>
                ))}
              </div>
            ...
            <div className="rating-actions">
              <button
                ...
                onClick={() => document.querySelector(".reviews-container").scrollIntoView({ behavior: "smooth" })}
              >
                View all reviews
              </button>
            </div>
          ...
          <div className="food-meta">
            <div className="food-meta-item">
              ...
              <div>
                ...
                <span className="meta-value">{food.food_type?.food_type || "Traditional"}</span>
              </div>
            </div>
            <div className="food-meta-item">
              ...
              <div>
                ...
                <span className="meta-value">{food.taste?.taste_type || "Savory"}</span>
              </div>
            </div>
          </div>
      ...
      <div className="reviews-container">
          ...
          <div className="rating-distribution">
            {[5, 4, 3, 2, 1].map((rating) => {
              ...
              return (
                <div
                  key={rating}
                  className={`rating-bar-item ${activeTab === rating.toString() ? "active" : ""}`}
        ...
        <div className="reviews-tabs-container">
          <div className="reviews-tabs">
            ...
            {[5, 4, 3, 2, 1].map((rating) => (
              <button
                key={rating}
                className={`tab-btn ${activeTab === rating.toString() ? "active" : ""}`}
                onClick={() => handleTabChange(rating.toString())}
              >
          ...
          {displayedReviews.length === 0 ? (
            ...
            <>
                  ...
                  <div key={review.id || index} className="review-card">
                    <div className="review-card-header">
                      <div className="reviewer-profile">
                        <div className="reviewer-avatar">{review.user_name?.charAt(0).toUpperCase() || "A"}</div>
                    ...
                    <div className="review-card-footer">
                      <button className="review-helpful-btn">
                        <span className="helpful-icon">👍</span> Helpful
                      </button>
              ...
              {displayedReviews.length <
                reviews.filter((review) => activeTab === "all" || review.rating === Number.parseInt(activeTab))
                  .length && (
        ...
        <div id="review-form" className="review-form-section">
          ...
          <form onSubmit={handleSubmitReview} className="review-form">
              ...
              <div className="rating-stars">
                {[...Array(5)].map((_, index) => {
                  ...
                  return (
                    <span
                      key={index}
                      className={`rating-star ${ratingValue <= (hoverRating || userRating) ? "filled" : ""}`}
            ...
            {!isLoggedIn && (
              <div className="form-group anonymous-form-group">
                <div className="anonymous-option">
                  ...
                  />
                  <label htmlFor="anonymous-review" className="checkbox-label">
                    Post as anonymous
                  </label>
                </div>
            ...
            <div className="form-actions">
              <button
                ...
                disabled={userRating === 0 || submitting || (!isLoggedIn && !anonymous)}
              >
